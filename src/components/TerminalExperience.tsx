...
export default function TerminalExperience({ cvData }: TerminalExperienceProps) {
  ...
  const addHistory = useCallback((content: React.ReactNode) => {
    setHistory(prev => [...prev, { id: Math.random().toString(36).substring(7), content }]);
  }, []);
  ...
  const printHelp = useCallback(() => {
    const commands = [
      ...
      { cmd: 'pdf', desc: 'Download ATS-optimized PDF resume' },
      { cmd: 'docx', desc: 'Download ATS-optimized DOCX resume' },
  ...
  const printAbout = useCallback(async (isBoot = false, isMountedFunc?: () => boolean) => {
    ...
    for (const line of heroLines) {
      if (isMountedFunc && !isMountedFunc()) return;
      addHistory(<div className="text-[#00ff41] whitespace-pre-wrap">{line}</div>);
    ...
    switch (trimmedCmd) {
        ...
        addHistory(
          <div className="mb-4 space-y-2">
            ...
            <div className="grid grid-cols-[180px_1fr] gap-y-1 text-[#8b949e]">
              <div className="text-[#00ffff]">Languages:</div><div>{cvData.programmingLanguages.join(', ')}</div>
              <div className="text-[#00ffff]">Frameworks:</div><div>{cvData.frameworks.join(', ')}</div>
              <div className="text-[#00ffff]">Databases:</div><div>{cvData.databases?.join(', ')}</div>
              <div className="text-[#00ffff]">Infrastructure:</div><div>{cvData.infrastructure?.join(', ')}</div>
            </div>
          </div>
        );
        break;
          ...
          <div className="mb-4 space-y-4">
            ...
            {cvData.workExperience.map((exp, i) => (
              <div key={i} className="pl-4 border-l border-[#00ff41]/30">
                <div className="text-[#00ffff] font-bold">{exp.company} <span className="text-[#8b949e] font-normal">| {exp.location}</span></div>
                <div className="space-y-2 mt-2">
                  {exp.positions.map((pos, pIdx) => (
                    <div key={pIdx} className="mb-3">
                      <div className="text-white">{'=> '} {pos.title} <span className="text-[#8b949e]">({pos.startDate} - {pos.endDate})</span></div>
                      {Array.isArray(pos.description) ? (
                        ...
                        <div className="text-[#8b949e] mt-1 pl-6">- {pos.description}</div>
                      )}
                      <div className="text-[#00ffff] text-sm mt-2 pl-6">Tech: <span className="text-[#8b949e]">{pos.skills.join(', ')}</span></div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      case 'projects':
        if (!cvData.projects || cvData.projects.length === 0) {
          addHistory(<div className="mb-4 text-[#8b949e]">No projects found.</div>);
          break;
        }
        addHistory(
          <div className="mb-4 space-y-4">
            ...
            {cvData.projects.map((proj, i) => (
              <div key={i} className="pl-4 border-l border-[#00ffff]/30 mb-4">
                <div className="text-[#00ffff] font-bold">
                  {proj.name}
                  {proj.url && <span className="text-[#8b949e] font-normal ml-2">[{proj.url}]</span>}
                </div>
                {Array.isArray(proj.description) ? (
                  ...
                  <div className="text-[#8b949e] mt-2 pl-4">- {proj.description}</div>
                )}
                <div className="text-[#00ffff] text-sm mt-2 pl-4">Tech: <span className="text-[#8b949e]">{proj.technologies.join(', ')}</span></div>
              </div>
            ))}
          </div>
        );
          ...
          <div className="mb-4 space-y-4">
            ...
            {cvData.education.map((edu, i) => (
              <div key={i} className="text-[#8b949e]">
                <span className="text-[#00ffff]">{edu.degree}</span> @ {edu.institution} ({edu.startDate} - {edu.endDate})
              </div>
            ))}
            {cvData.certifications && cvData.certifications.length > 0 && (
              <div className="mt-2">
                ...
                {cvData.certifications.map((cert, i) => (
                  <div key={i} className="text-[#8b949e] pl-4">- {cert.name}</div>
                ))}
              </div>
            )}
          </div>
      ...
      case 'contact':
        addHistory(
          <div className="mb-4 space-y-1">
            ...
            <div className="text-[#8b949e]"><span className="text-[#00ffff]">Email:</span> {cvData.contactInfo.email}</div>
            {cvData.contactInfo.phone && <div className="text-[#8b949e]"><span className="text-[#00ffff]">Phone:</span> {cvData.contactInfo.phone}</div>}
            <div className="text-[#8b949e]"><span className="text-[#00ffff]">GitHub:</span> {cvData.contactInfo.github}</div>
            <div className="text-[#8b949e]"><span className="text-[#00ffff]">LinkedIn:</span> {cvData.contactInfo.linkedin}</div>
          </div>
        );
        break;
      case 'gui':
      case 'static':
        addHistory(<div className="mb-4 text-[#00ff41]">Launching Graphical User Interface...</div>);
        setTimeout(() => {
          router.push('/static');
        }, 1000);
        break;
      case 'pdf':
        addHistory(<div className="mb-4 text-[#00ff41]">Generating ATS-optimized PDF...</div>);
        window.open('/api/generate-pdf', '_blank');
        break;
      case 'docx':
        addHistory(<div className="mb-4 text-[#00ff41]">Generating ATS-optimized DOCX...</div>);
      ...
      case 'clear':
        ...
        break;
      case 'sudo':
        addHistory(<div className="mb-4 text-red-500">carlos is not in the sudoers file. This incident will be reported.</div>);
        break;
      case 'whoami':
        addHistory(<div className="mb-4 text-[#8b949e]">guest_user</div>);
        break;
      default:
        addHistory(<div className="mb-4 text-red-500">Command not found: {trimmedCmd}. Type &apos;help&apos; to see available commands.</div>);
    }
  };
